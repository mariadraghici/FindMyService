...
import HeaderPositionContext from './context/HeaderPosition';

const Header = () => {
  const navigate = useNavigate();
  const {user, setUser} = useContext(ProfileContext);
  const {headerPosition} = useContext(HeaderPositionContext);
  ...
  return (
      <Box sx={{ flexGrow: 1 }}>
        <AppBar position={headerPosition} color='transparent' >
          <Container>
            <Toolbar>
              <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
                <img src={logo} alt="logo" className="img-width" />
              </Typography>
              ...
              {user && user.role === 2 && <Button color="inherit" ><Link to='/service/offers' style={{ color: '#FFF' }}>Oferte</Link></Button>}
